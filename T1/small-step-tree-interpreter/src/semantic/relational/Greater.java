...
public class Greater extends BooleanExpression {
    ...
    public Greater(ArithmeticExpression exp1, ArithmeticExpression exp2) {
        this.exp1 = exp1;
        this.exp2 = exp2;
    }
    ...
    public AbstractSyntaxTree smallStep(Environment environmentState) {
        ...
        if (!(this.exp1 instanceof Literal)) {
            return new Greater((Literal) this.exp1.smallStep(environmentState), this.exp2).smallStep(environmentState);
        } else if (!(this.exp2 instanceof Literal)) {
            return new Greater(this.exp1, (Literal) this.exp2.smallStep(environmentState)).smallStep(environmentState);
        } else {
